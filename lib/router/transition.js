...
function Transition(router, intent, state, error) {
  ...
  if (error) {
    this.promise = config.Promise.reject(error);
    return;
  }
  ...
  if (state) {
    ...
    this.promise = state.resolve(checkForAbort, this)['catch'](function(result) {
      if (result.wasAborted || transition.isAborted) {
        ...
        transition.abort();
        return config.Promise.reject(result.error);
      }
    }, promiseLabel('Handle Abort'));
  } else {
    this.promise = config.Promise.resolve(this.state);
    this.params = {};
  }

  function checkForAbort() {
    if (transition.isAborted) {
      return config.Promise.reject(undefined, promiseLabel("Transition aborted - reject"));
    }
  }
}
...
Transition.prototype = {
  ...
  followRedirects: function() {
    ...
    return this.promise['catch'](function(reason) {
      ...
      return config.Promise.reject(reason);
    });
  },
